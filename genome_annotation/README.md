...
Практикум для курсов NGS. Студенты учатся аннотировать бактериальный геном с помощью стандартных биоинформатических инструментов на HPC-системе.
...
- **SSH-доступ к HPC** (вычислительный узел, не login node)
- **Базовые навыки командной строки** Linux (`cd`, `ls`, `less`, `head`, `grep`)
- **Conda/micromamba** для управления окружением
- **Ресурсы:** ~500 MB дискового пространства, 4-8 CPU cores (рекомендуется)

> **Для преподавателей:** См. `../shared/HPC_SETUP.md` для детальных инструкций по настройке HPC-окружения.
...
Для *B. subtilis* 168 (геном ~4.2 Мб):
- **~4200 CDS** (белок-кодирующих генов)
- **~85-90% генов** с DIAMOND hits (гомология в Swiss-Prot)
- **~95% генов** с Pfam доменами
- **~10 rRNA, ~80 tRNA**

См. `docs/expected_outputs.md` для подробных метрик.
...
1. **Проверьте результаты:** `bash scripts/40_sanity_checks.sh` покажет итоговую статистику
2. **Изучите примеры:** `bash scripts/50_pick_3_genes.sh` выберет 3 гена для обсуждения
3. **Домашнее задание:** См. `docs/homework_01.md` для полного задания (2 части: functional profile summary + case investigation)

**Полезные команды:**
```bash
make all             # Запустить весь workflow
make clean           # Удалить outputs/ (сохраняет data/)
make clean-all       # Удалить всё (включая базы данных)
